...
class Plant:
    ...
    def plant_id(self):
        try:
            response = requests.get(
                f"{self.url}species-list?key={self.api_key}&q={self.name}")  # Make a GET request to the API
            response.raise_for_status()  # This will raise an HTTPError for bad responses
            data = response.json()  # Parse the JSON response from the HTTP request and store it in the 'data' variable
            if 'data' in data and data[
                'data']:  # checks if the key 'data' exists in the list dictionary and this value is not empty or None.
    ...
    def complete_details(self):
        if self.id:
            try:
                response = requests.get(
                    f"{self.url}species/details/{self.id}?key={self.api_key}")  # API endpoint to get detailed information using the plant ID
                response.raise_for_status()  # This will raise an HTTPError for bad responses
                ...
                if 'data' in care_info and care_info['data']:
                    ...
                    for section in sections:
                        if section['type'] in types:
                            result.append(
                                f"{section['type'].capitalize()} Description: {section.get('description', 'N/A')}")
                    return result
                else:
...
def youtube_search(query, API_KEY):
    ...
    request = service.list(q=f'{query}', part='snippet', type='video', maxResults=2, order='rating',
