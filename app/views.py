...
@login_required  # Can only be accessed if a user is logged in
def dashboard():
    if request.method == 'POST':
        # Get the form data
...
def plant_care():
    ...
    # query = None
    # if request.method == 'POST':
    query = request.form.get('query')  # Get a query from the Search box
    if query:
        # Plant API
        ...
        if plant_data and video_ids:
            ...
            return render_template('plant_care.html',
                                   error=f"Incorrect plant name :{query}. Please enter a valid plant name ")
    else:
        return render_template('plant_care.html')
